...
<tiles:insert definition="standard">
	...
	<!--
		function edit(i) {
			document.location="<%=response.encodeURL(request.getContextPath()+"/system/management/role/edit.do?id=")%>"+i;
		}
	//-->
	</script>
	...
	<html:form action="/system/management/role" styleClass="list" style="width: 600px;">
				...
				<tr>
					...
					<td>
						...
						<html:select property="admin" styleId="locked">
							<html:option value="<%=RoleDAO.AdminSelect.ALL.toString()%>">&nbsp;</html:option>
							<html:option value="<%=RoleDAO.AdminSelect.ADMIN.toString()%>"><bean:message key="default.boolean.true"/></html:option>
							<html:option value="<%=RoleDAO.AdminSelect.NOT_ADMIN.toString()%>"><bean:message key="default.boolean.false"/></html:option>
						</html:select>
					</td><%
					}%>
				</tr>
		...
		<fieldset class="listAction">
			<html:link action="/system/management/role/edit" titleKey="image.icon.add.text">
		...
		<fieldset class="list">
			...
			<table class="result">
				...
				<tr>
					...
					if(admin) { %>
					<th><bean:message key="entity.system.role.admin"/></th><%
					}%>
				</tr>
